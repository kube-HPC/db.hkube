...
class Collection {
    ...
    async create(doc, { applyId = false, checkKeys = false } = {}) {
        try {
            const response = await this.collection.insertOne({ ...doc }, { checkKeys }); // to prevent mutate the doc (_id)
            if (applyId) {
     ...
     */
    async createMany(array, { throwOnConflict = false, ordered = false, checkKeys = false } = {}) {
        try {
