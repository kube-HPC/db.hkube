...
class Collection {
     ...
     * @returns {Promise<T>}
     */
    async create(doc, { applyId = false, checkKeys = true } = {}) {
        try {
            // to prevent mutate the doc (_id)
    ...
    async updateOne({ filter, query, session, upsert = false }) {
        const result = await this.collection.updateOne(filter, query, { upsert, session });
        return result;
    }
