...
const { createProjection, sortOrderHelper } = require('./utils');
...
class Collection {
    ...
    async _createIndex(index) {
        ...
        const { unique, ...props } = fields;
        const indices = sortOrderHelper(props);
        await this.collection.createIndex(indices, { name, unique });
    }
    ...
    /**
     * @param {T[]} docs
     * @param {{ ordered?: boolean }} settings
     */
    async createMany(docs, { ordered = false } = {}) {
        try {
    ...
    fetchAll({ query = {}, fields = {}, excludeId = false, sort = null, limit = 0 } = {}) {
        const projection = createProjection(fields, excludeId);
        const sortParams = sortOrderHelper(sort);
