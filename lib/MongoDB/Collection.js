...
class Collection {
    ...
    constructor(db, mongoClient, collection) {
        ...
        this.collection = db.collection(`${collection.name}s`);
    ...
    async init() {
        if (this.index) {
            this._createUniqueIndex(this.index);
        }
    }
    ...
    async fetch(query, { excludeId = false, included, excluded } = {}) {
        let entry;
        const projection = createProjection(included, excluded, excludeId);
    ...
    fetchAll({ query = {}, excludeId = false, included, excluded, sort, limit = 0 } = {}) {
        const projection = createProjection(included, excluded, excludeId);
