...
class Collection {
    ...
    async create(doc, { applyId = false, checkKeys = false } = {}) {
        try {
            ...
            const response = await this.collection.insertOne({ ...doc }, { checkKeys });
            if (applyId) {
    ...
    /**
     * @param {T[]} array
     * @param {{ ordered?: boolean; throwOnConflict?: boolean; returnDocs?: boolean }} settings
     */
    async createMany(
        array,
        { throwOnConflict = false, ordered = false, returnDocs = false, checkKeys = false } = {}
    ) {
        try {
            const docs = cloneDeep(array); // to prevent mutate the docs (_id)
            const result = await this.collection.insertMany(docs, { ordered, checkKeys });
            return {
                inserted: result.insertedCount,
                ...(returnDocs
                    ? { docs: docs.map(entry => ({ ...entry, id: entry._id.toHexString() })) }
                    : {}),
            };
        } catch (error) {
            if (throwOnConflict) {
    ...
    async updateOne({ filter, query, session, upsert = false, checkKeys = false }) {
    ...
    async replaceOne({ filter, replacement, upsert = false, checkKeys = false }) {
        const result = await this.collection.replaceOne(filter, replacement, { upsert, checkKeys });
        return result;
    }
