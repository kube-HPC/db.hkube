...
class DataSources extends Collection {
    ...
    async create({ name }) {
        ...
        await this.transaction(async () => {
            const existingEntry = await this.fetch({ name });
    ...
    fetch(query, { fields = {}, sort = null }) {
        return super.fetch(query, { allowNotFound: false, fields, sort });
    }
    ...
    async upsertSnapshot({ name, id, snapshot }) {
        await this.transaction(async () => {
            ...
            const snapshots = entry.snapshots
                .filter(item => item.name !== snapshotName)
                .concat(snapshot);
