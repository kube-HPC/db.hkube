    ...
    async create({ name }) {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                //                           ^
                //                           |
                // but you still need an index for better performance
    ...
    async uploadFiles({ name, id, filesAdded = [], filesDropped = [] }) {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                const filesDroppedSet = new Set(filesDropped.map(item => item.name));
                const filesAddedSet = new Set(filesAdded.map(item => item.name));
