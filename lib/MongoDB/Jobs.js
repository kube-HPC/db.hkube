...
class Jobs extends Collection {
    ...
    async watchStatus({ jobId }, cb) {
        ...
        const watchCursor = await super.watch([pipeline], (doc) => {
            const { status } = doc.updateDescription?.updatedFields || {};
            if (status) {
                const { _id, ...rest } = doc.fullDocument.status;
                const job = { ...rest, ...status };
                cb(job);
            }
        });
    ...
    async watchResult({ jobId }, cb) {
        ...
        const watchCursor = await super.watch([pipeline], (doc) => {
            const { result } = doc.updateDescription?.updatedFields || {};
            if (result) {
                const { _id, ...rest } = doc.fullDocument.result;
